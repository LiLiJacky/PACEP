...
def nfa(regex, min_window_time, max_window_time, data_source, rate):
    constraints_dict = {
        'value_constrain':[],
        'time_constrain': [
            {'variables': ['A', 'B[i]'], 'min_time': min_window_time, 'max_time': max_window_time}
        ],
        'type_constrain': [
...
if __name__ == "__main__":
    ...
    data_source = '../../data/simulate_data/stock-small.stream'
    ...
    df.to_csv("../../output/experiment/compare/ab+.csv", index=False)

    print("Results saved to nfa_statistics.csv")
