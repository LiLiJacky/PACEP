...
if __name__ == "__main__":
    pattern_names = ["stock_detection"]
    # pattern_names = ["hadoop_detection","stock_detection", "flash_crash_detection"]
    pattern_list = {
        "lng_rollover": "A{5,} B{5,} C",
        "hadoop_detection":  "A B{2,} C",
        "stock_detection": "A{2,} B{2,}",
    ...
    for pn in pattern_names:
        ...
        for i in range(1):  # 10轮实验
            ...
            output_name = data_source[pn]

            #max_window_times = [10, 20, 30, 40, 50, 60, 80, 100, 120]
            max_window_times = [4, 6, 8, 10, 12, 14, 15, 16, 17, 18]
            # max_window_times = [200]

            min_window_time = 0

            data_source = '../../data/compare/' + output_name + '.csv'

            regex = pattern_list[pn]

            for max_window_time in max_window_times:
                if pn in ["stock_detection", "flash_crash_detection"]:
                    max_window_time = max_window_time * 3600 * 24
                pattern_constrain_list = {
                    ...
                    "stock_detection": {
                        'value_constrain': [
                            ...
                            {'variables': ['A'], 'expression': '0 <= last(A) - 1.05 * first(A) <= 1000'},
                            {'variables': ['B', 'B[i]'], 'expression': '0 <= B[i] - last(B) <= 1000'},
                            {'variables': ['A', 'B'],
                    ...
                    "flash_crash_detection": {
                        'value_constrain': [
                            {'variables': ['A'], 'expression': '0.3 <= sum_square_difference(A) <= 1000'},
                ...
                constrain_dict = pattern_constrain_list[pn]
        ...
        # 保存为 CSV 文件
        output_path = f"../../output/{pn}_real_result.csv"
