...
if __name__ == "__main__":
    ...
    for i in range(10):  # 10轮实验
        ...
        run_results = []
        for b in np.arange(0.2, 1.0, 0.2):
            print(time.time())
            a = 1 - b  # b 是 a 的补数
            event_dict = {"a": a, "b": b, "c": a}
            output_name = f"ab+c_{a:.1f}_{b:.1f}"
    ...
    # 保存为 CSV 文件
    output_path = "../../output/ab+c_test_result.csv"
    ...
    # 保存图片
    output_image_path = "../../output/ab+c_handle_time.png"
    plt.savefig(output_image_path)

    # 绘制第二张图：Average Memory Usage vs Window Time
    plt.figure(figsize=(12, 8))
    ...
    # 保存并显示第二张图
    output_image_path_memory_usage = "../../output/a+b+_memory_usage.png"
    plt.savefig(output_image_path_memory_usage)
    plt.show()