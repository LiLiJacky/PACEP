...
def test(regex, min_window_time, max_window_time, data_source, rate, lazy_model, lazy_calculate_model, contiguity_strategy):
    constraints_dict = {
        'value_constrain': [
            ...
            {'variables': ['B', 'C'], 'expression': '0.5 <= C - B <= 1000'},
        ],
        'time_constrain': [
...
if __name__ == "__main__":
    ...
    for i in range(1):  # 10轮实验
        ...
        for a in [0.2]:
            ...
            # output_name = f"minimal_test"

            # 调用生成数据的方法
            generate_data_with_pandas_sorted(event_dict, 1000, output_name, start_time, time_interval_seconds=1)

            #max_window_times = [2, 4, 6, 8, 10, 12, 14, 16]
            max_window_times = [10, 20, 30, 40, 50, 100, 150, 200]
            #max_window_times = [8]
            min_window_time = 0
            ...
            for max_window_time in max_window_times:
                ...
                for event_selection_strategy, contiguity_strategy in contiguity_strategys.items():
                    ...
                    # Lazy 优化测试
                    lazy_increment_handle_time, lazy_increment_matches, lazy_increment_memory = test(regex,
                                                                                                     min_window_time,
                                                                                                     max_window_time + 1,
                                                                                                     data_source, rate,
                                                                                                     True,
                ...
                if time.time() - frequency_start_time > 600:
                    break
    ...
    # 保存为 CSV 文件
    output_path = "../../output/minimal_test_result_a+bc.csv"
