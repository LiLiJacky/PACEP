    ...
    try:
        while True:
            ...
            if data_item:
                ...
                if matches:
                    for match in matches:
                        hashid = table_tool.ensure_hashable(match)
                        if not bf.check(hashid):
                            bf.add(hashid)
                            matcount += 1
                        lista = []
                        for a in match[0]['A']:
                        ...
                        stringId = table_tool.ensure_hashable(match)
                        if stringId not in total_matched:
                            record.append({"a": lista, "b": listb, "c": listc})
                            total_matched.add(stringId)
        ...
        #print(record)
        print(len(total_matched))
        print("hashnums" + str(matcount))

        return last_handle_time - modify_begin_time, len(total_matched), record
    ...
    for i in range(1):  # 10轮实验
        ...
        for max_window_time in max_window_times:
            ...
            for event_selection_strategy, contiguity_strategy in contiguity_strategys.items():
                # # NFA 测试
                # nfa_handle_time, nfa_matches = test(regex, min_window_time, max_window_time, data_source, rate,
                #                                     False, "", contiguity_strategy)

                # Lazy 测试
                lazy_handle_time, lazy_matches, lazy_record = test(regex, min_window_time, max_window_time + 1, data_source, rate,
                                                      True, "TABLECALCULATE", copy.deepcopy(contiguity_strategy))

                # Lazy 优化测试
                lazy_increment_handle_time, lazy_increment_matches, lazy_increment_record = test(regex, min_window_time, max_window_time + 1, data_source, rate,
                                                      True, "INCREMENTCALCULATE", copy.deepcopy(contiguity_strategy))

                if len(lazy_record) != len(lazy_increment_record):
                    lazy_record_list = []
                    for result in lazy_record:
                        lazy_record_list.append(table_tool.ensure_hashable(result))
                    lazy_increment_record_list = []
                    for result in lazy_increment_record:
                        lazy_increment_record_list.append(table_tool.ensure_hashable(result))
                    print(len(lazy_increment_record))
                    print(len(lazy_record))
                    for result in lazy_increment_record_list:
                        if result not in lazy_record_list:
                            print("lazy 少了")
                            print(result)
                    for result in lazy_record_list:
                        if result not in lazy_increment_record_list:
                            print("lazy incremtne 少了")
                            print(result)
                # NFA 结果
                # nfa_result = {
